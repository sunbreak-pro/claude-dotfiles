...
| 重量級 1 タスク、**独立単位へ並列分解可**（2〜5 unit）、PR は 1 本に統合したい                   | **`ultracode`**    | lead-pipeline の ultracode モード（references/ultracode-mode.md） |
...
- **Claude は `/goal` `/batch` `/loop` を Bash 等で実行しない**。必ずユーザーが貼る前提でコマンド文字列を提示するだけ。
...
- **`/batch` は worktree を量産し各ワーカーが自動 `gh pr create` する**。実行前に必ず:
  - main / master / production への直 push にならないか（[[feedback_branch_protection]] / pre-push hook 前提）
...
- 破壊的になりうる戦略（大量 PR / force push を含む rebase 戦略）は [[feedback_destructive_git_confirmation]] に従い実行直前再確認を明記。
