...
| --------------------------------------------------------------- | ------- | -------------------------------------------------- |
| 「作業開始」「session start」「再開する」「次のタスクやる」     | START   | 作業開始フロー（active-sessions 競合チェック込み） |
...
- `.claude/active-sessions/` に自分以外のセッションファイルがあるときは、各ファイルを Read して cwd / branch / 担当領域を突き合わせ、共有ファイル（memory/history）や同一ブランチに触る他チャットがいないかを**このスキル内で直接確認**する（専用エージェント multi-session-coordinator は 2026-08-13 廃止。照会のために別エージェントは起動しない）
- 競合が疑われる場合はユーザーに提示して判断を仰ぎ、解消されてから以降の Step に進む
...
- INDEX.md の鮮度が実ファイルより古く、per-chat ファイルを直接 Read する必要があるとき
